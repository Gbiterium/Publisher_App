  ...
  <div class="d-flex flex-column width50 mx-auto">
    <div class="row text-center my-3">
      <div class="col text-left">
        <h2 class="text-left font-family_brown font-weight-bold">Sign in to Slate</h2>
    ...
    <div class="row my-2 googleresponsive">
      <!-- <div class="col"> -->
    ...
    <ValidationObserver v-slot="{ handleSubmit }">
      <form class="googleresponsive" @submit.prevent="handleSubmit(submitForm)">
        <div class="row">
          ...
          <div class="col-12">
            <div class="form-group">
              <BaseButton watch-request class="w-100 btn-lg">Login</BaseButton>
            </div>
          </div>
        </div>
      </form>
  ...
  methods: {
    async submitForm() {
      try {
        ...
        if(this.$route.query.desktop === 'true'){
          this.$router.push(`/auth/desktop-auth?desktop=true&protocol=${this.$route.query.protocol}`)
        } else if(localStorage.getItem('onboardingLogIn') === 'true') {
            this.$router.push('/onboarding/about-institution')
            localStorage.removeItem('onboardingLogIn')
        } else{
          await this.redirect();
        }
    ...
    async googleLogin(response) {
      try {
        ...
        this.$refs.authProcessModal.setText("Redirecting...");
        if(localStorage.getItem('onboardingLogIn') === 'true') {
            this.$router.push('/onboarding/about-institution')
            localStorage.removeItem('onboardingLogIn')
        } else {
        await this.redirect();
