<template>
  <div>
    <Analytics @filter-date="handleDateFilter" @filter-items="handleFilter">
      ...
      <template #subtitle>
        <div>
          View the total amount of pages read for this month, you can adjust how
          data is displaced using the filters belows.
        </div>
      </template>
  ...
  methods: {
    ...
    async getEarning() {
      try {
        ...
        const page_reads = data.data.map((el) =>
          el.books.reduce((a, el) => a + el.total_reads, 0)
        );
