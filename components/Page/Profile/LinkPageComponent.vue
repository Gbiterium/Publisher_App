<template>
  <div class="container vh-95">
    ...
    <template v-else>
      ...
      <slot name="card">
        <div v-if="data.length > 0" class="row">
          <div v-for="(school, index) in data" :key="index" class="col-md-6 my-3 my-md-0">
    ...
    <PageProfileLinkModal @button:click="link({ entity, schoolCode, linkCode })">
      <template #title>
        <h6 class="font-weight-bold mb-1">Link {{ entity }} to school.</h6>
        <p class="subtext">
          Enter the link to join your
          <span v-if="entity == 'family'" class="subtext">child's</span> school
