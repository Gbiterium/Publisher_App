      ...
      <div class="navbar px-3 px-md-5">
        ...
        <div class="d-flex align-items-center flex-md-wrap">
          ...
          >
            ...
            <template v-if="studentArray.length > 0">
              <div class="dropdown account-dropdown">
                <a
                  ...
                  aria-expanded="false"
                  @click="
                    toShow($event, studentArray.length > 0 ? true : false)
                  "
                  >Student
                  <span
            ...
            <template v-if="schoolArray.length > 0">
              <div class="dropdown account-dropdown">
                ...
                <div
                  ...
                  aria-labelledby="dropdownMenuButton"
                >
                  <template v-for="(school, index) in schoolArray">
                    <div :key="index" class="school-dropdown" @click="store(school)">
                      <nuxt-link
                      v-if="!school.activation_status"
                        class="dropdown-item"
                        :to="'/onboarding/' + school.code + '/follow-up'"
            ...
            <!-- <div class="nav-item">
            <a href="#" class="nav-item-link">Shops</a>
          </div> -->
          </nav>
          <div class="ml-md-4">
            <svg
        ...
        <div v-if="fullScreenMenu" class="display mobilecontainer">
          ...
          <div class="innermobcontainer px-2">
            ...
            <nuxt-link class="dropdown-item" to="/onboarding/"
                    >Add School</nuxt-link
                  >
            <nuxt-link class="fs-16 dropdown-item my-3" to="/auth/link/student"
              >Student Settings</nuxt-link
            >
            <nuxt-link class="fs-16 dropdown-item my-3" to="/auth/link/parent"
              >Parent Settings</nuxt-link
            >
            <nuxt-link class="fs-16 dropdown-item my-3" to="/auth/link/school"
              >School Settings</nuxt-link
            >
          ...
          <div class="py-3 lightborder-bottom mx-4">
            ...
            <div class="mt-2">
              <div class="py-2 submenu">
                <template v-for="(school, index) in schoolArray">
                  <div :key="index" @click="store(school)">
...
export default Vue.extend({
  ...
  async created() {
    try {
      if (this.$store.state.student.schools.length <= 0) {
        ...
        this.$store.commit('school/SET', this.menuResponseData.teachers)
        this.$store.commit('parent/SET', this.menuResponseData.children_school)

        const studentArray = this.menuResponseData.schools.find(
          (item: any) => item.key === 'my_school'
        ).data
  ...
  methods: {
    ...
    store(school: any) {
      this.$store.commit('administration/SET_SCHOOL', school)
    },
    async getMenuData() {
      try {
